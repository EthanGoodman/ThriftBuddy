...
async def extract_from_files(
    ...
    try:
        ...
        try:
            main_image_vecs = await image_bytes_to_embeddings_multicrop_async(main_bytes, crops=MAIN_CROPS)
        ...
        if mode in ("active", "both") and active_items:
            await embed_thumbnails_for_items(active_items, max_items=EMBED_MAX_INITIAL, concurrency=THUMB_CONCURRENCY)
        if mode in ("sold", "both") and sold_items:
            await embed_thumbnails_for_items(sold_items, max_items=EMBED_MAX_INITIAL, concurrency=THUMB_CONCURRENCY)

        # ---- Rerank (INITIAL ONLY), just to produce signal and pick best title ----
        active_ranked = None
        sold_ranked = None
        if mode in ("active", "both") and active_items:
            active_ranked = rerank_items_by_image_similarity(
                active_items, main_image_vecs, threshold=RERANK_THRESHOLD, keep_top_k=TOPK_SIGNAL
            )
        if mode in ("sold", "both") and sold_items:
            sold_ranked = rerank_items_by_image_similarity(
        ...
        if refined_query:
            ...
            refined_candidates = {
                "active_top50": [slim_item(it) for it in active_items_ref[:50]] if active_items_ref else [],
                "sold_top50": [slim_item(it) for it in sold_items_ref[:50]] if sold_items_ref else [],
            }
        ...
        response: Dict[str, Any] = {
            ...
            "final_candidates": refined_candidates
                if refined_candidates is not None
                    ...
                    "active_top50": [slim_item(it) for it in active_items[:50]] if active_items else [],
                    "sold_top50": [slim_item(it) for it in sold_items[:50]] if sold_items else [],
                },
        }
